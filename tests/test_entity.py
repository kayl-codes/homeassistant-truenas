...
def test_available_is_false_while_backing_job_is_failing() -> None:
    ...
    ``_data`` itself is still populated with the last-good snapshot."""
    coordinator = make_coordinator(data={"disk": {"d1": {"guid": "g1"}}})
    coordinator.is_data_path_failing = lambda data_path: data_path == "disk"
    entity = _make_entity(
        uid="d1", data={"guid": "g1"}, description=_REF_DESC, coordinator=coordinator
    )
    assert entity.available is False
...
def test_available_keyless_entity_unavailable_while_data_path_failing() -> None:
    """A keyless entity on a failing data_path goes unavailable even though
    the still-populated last-good snapshot alone would report available."""
    coordinator = make_coordinator()
...
def test_device_info_non_system_group_uses_via_device_id_when_supported() -> None:
    ...
    entity = _make_entity(description=desc)
    with patch(
        "custom_components.truenas_ce.entity._supports_via_device_id",
        return_value=True,
    ):
        info = entity.device_info
