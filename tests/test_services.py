...
Like ``test_config_flow_flows.py``, this module drives a real
``HomeAssistant`` instance and therefore needs
``pytest-homeassistant-custom-component`` (see that module's docstring for
why it cannot be installed on this repo's Windows dev machine); it is only
collected in CI and skipped locally via ``conftest.py``.
"""
...
async def _add_loaded_entry(
    ...
    with patch(
        "custom_components.truenas_ce.async_setup_entry",
