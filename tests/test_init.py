...
from custom_components.truenas_ce.helper import GB_SCALED_UNITS
...
def test_migrate_data_size_units_skips_fixed_mib_descriptions() -> None:
    """DATA_SIZE sensors with a fixed MiB suggestion (app memory/block I/O) must
    not be forced onto the GB/GiB scale -- only GB_SCALED_UNITS-suggested ones
    opt in.
    """
    coordinator = MagicMock()
    coordinator.ds = {}
...
def test_migrate_data_size_units_resets_fixed_mib_descriptions() -> None:
    """Descriptions excluded from the migration loop must instead be routed to
    the stale-unit reset path, not silently skipped altogether.
    """
    coordinator = MagicMock()
    coordinator.ds = {}
    entry = _config_entry(options={"data_unit": "GiB"})
